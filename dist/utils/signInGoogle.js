...
const signInGoogle = (webClientId) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        mayo_logger_1.Logger.info("Configuring GoogleSignin with webClientId.", { webClientId });
        ...
        mayo_logger_1.Logger.info("Checking availability of Google Play Services.", { playServicesAvailable });
        const signInResult = yield google_signin_1.GoogleSignin.signIn();
        mayo_logger_1.Logger.info("User signed in using Google.", { userId: signInResult.user.id }); // Log only user ID for privacy reasons
        if (webClientId) {
            yield async_storage_1.default.setItem('webClientId', webClientId); // Store webClientId for convenience during logout
            mayo_logger_1.Logger.info("Stored webClientId in AsyncStorage.");
        }
        ...
        if (error.code === google_signin_1.statusCodes.SIGN_IN_CANCELLED) {
            mayo_logger_1.Logger.warn("User cancelled the Google sign-in process.");
        }
        else {
            mayo_logger_1.Logger.error("Error during Google sign-in.", { message: error.message, errorCode: error.code });
        }
    }
});
